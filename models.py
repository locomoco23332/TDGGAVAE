...
class P2GEncoder(nn.Module):
    ...
    def encode(self, t1, t2, t3, t4, t5):
        data = torch.stack((t1, t2, t3, t4, t5), dim=1)  # Stack tensors for GRU
        ...
        mu = self.mu(torch.cat((out4, t2,t3, t4), dim=1))
        logvar = self.logvar(torch.cat((out4, t2, t3, t4), dim=1))
...
class TDGAVAE(nn.Module):
    def __init__(self, tracker_size, condition_frame,encode_hidden_size, latent_size,latent_size2, decode_hidden_size, output_size1, output_numframe1, output_size2, output_numframe2):
        ...
        self.output_numframe1 = output_numframe1
        self.output_numframe2 = output_numframe2
        self.gru=nn.GRU(tracker_size,tracker_size,num_layers=1,batch_first=True)
        self.encoder1 = P1Encoder(tracker_size, encode_hidden_size, latent_size)
        self.encoder12 = P12Encoder(tracker_size,encode_hidden_size,latent_size)
        self.encoder13 = P13Encoder(tracker_size,encode_hidden_size,latent_size)
        self.diffusion = DenoiseDiffusion(3*latent_size,latent_size,noise_steps=10000)
...
class TDGGAVAE(nn.Module):
    def __init__(self, tracker_size, condition_frame,encode_hidden_size, latent_size,latent_size2, decode_hidden_size, output_size1, output_numframe1, output_size2, output_numframe2):
        ...
        self.output_numframe2 = output_numframe2
        self.gru=nn.GRU(tracker_size,tracker_size,num_layers=1,batch_first=True)
        self.encoder1 = P1GEncoder(tracker_size, encode_hidden_size, latent_size)
        self.encoder12 = P12GEncoder(tracker_size,encode_hidden_size,latent_size)
        self.encoder13 = P13GEncoder(tracker_size,encode_hidden_size,latent_size)
        self.diffusion = DenoiseDiffusion(3*latent_size,latent_size,noise_steps=10000)
        self.diffusion2 = DenoiseDiffusion2(latent_size,latent_size,noise_steps2=10000)
        self.gruflow=nn.GRU(latent_size*5,latent_size*5,num_layers=1,batch_first=True)
        self.gruflow2=nn.GRU(latent_size*3,latent_size*3,num_layers=1,batch_first=True)
